...
void send_hello(int sockfd, char* name, int fd){
  ...
  char buff_out[BUFFER_NAME_SIZE + BUFFER_DATE_SIZE + strlen(has_joined)];
  char time [BUFFER_DATE_SIZE];
  ...
  wrap_message(buff_out, time, trim_name(new_name), null_string);
...
int send_name(char* name, int sockfd){
  size_t len = strlen(name);
  char new_name [len];
  strncpy(new_name, name, len);

  int b = write(sockfd, trim_name(new_name), len);
  if(b <= 0) return 1;
  return 0;
}
...
void* send_message(void* usr_info){
  ...
  while (1)
  {
    ...
    if(strlen(message)>1)//if is not void
    {
      ...
      int bye_write = write(sockfd, message_wrapped, strlen(buffer));
      if (bye_write < 0)
           error("ERROR writing to socket", sockfd, fd_local_log);
