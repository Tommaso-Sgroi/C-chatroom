...
void append_string_message_to_send(char*string, int len, int client_fd, char* addr){
  ...
  printf("segnalo al consumatore il messaggio\n");
  ...
  printf("segnalato l'arrivo del messaggio al consumatore\n");
}
...
void* run_consumer(void* null) {
  ...
  while (1)
  {
    printf("\tprendo il lock sui messaggi da mandare\n");
    ...
    printf("\tmi metto in attesa della signal\n");
    ...
    */
    ...
    while(message_to_send_struct.message_to_send->first)//itero sulla linkedlist: while has next, ed è certo avere almeno 1 elemento
    {
      ...
      printf("salvo il messaggio nel global log\n");
    ...
    printf("\ttolgo il lock sui message to send\n");
